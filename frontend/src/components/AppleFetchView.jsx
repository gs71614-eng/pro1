...
export function AppleFetchView({ onSuccess, setLoading, loading, setError }) {
  ...
  return (
    <div className="glass-panel p-8 transition-all duration-300 border-[#64748b]/30 hover:border-[#64748b] shadow-lg">
      <h2 className="text-xl font-bold mb-6 flex items-center gap-2 text-white">Apple App Store Fetcher</h2>
      <form onSubmit={handleFetch} className="flex flex-col gap-5">
        <div>
          <label className="block text-sm font-medium text-gray-300 mb-1">Numeric App ID</label>
          <input type="text" value={appId} onChange={(e) => setAppId(e.target.value)} placeholder="e.g. 284882215" className="w-full bg-surface/80 border border-gray-600 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-[#64748b]" required />
          <p className="text-xs text-gray-500 mt-1">Provide the numeric string found in the App Store URL (id...)</p>
        </div>
        ...
        <button type="submit" disabled={loading} className="mt-4 w-full bg-[#64748b] hover:bg-slate-600 text-white font-semibold py-3 rounded-xl transition-all flex justify-center items-center gap-2 disabled:opacity-50">
